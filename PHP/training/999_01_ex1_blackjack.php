...
define( "FLG_DEBUG", true ); // 디버그 파일 확인용 (999_2_deck.txt , hand.txt 를 true 하면 생성)
...
class Deck
{
	...
	private function set_deck( $param_arr_card_num, $param_arr_card_shape )
	{
		...
		if( FLG_DEBUG )
		{
			DebugBJ::put_file( FILE_NAME_DECK, $this->arr_deck, "w" );  // 디버그 false일때 작동 안함
		}
			// ************** DEBUG **************
	}
...
class Dealer extends Player
{
	...
	public function set_var( &$param_obj_deck )
	{
		...
		{
			parent::set_var( $param_obj_deck);  // 부모의 딜러 덱을 그대로 가져와서 사용한다
		}
	}
}
...
class Play
{
	...
	public function print_result( $param_arr, $param_int_result, $param_str_con = "" )
	{
		...
		echo $str;
		$this->obj_user->print_open_card(); // 플레이어의 패를 보여주는 메소드
		echo " / ";
	...
	public function open_player_card()
	{
		...
		if( FLG_DEBUG )
		{
			$this->obj_dealer->print_open_card();             // 디버그여서 true일때 딜러 패 보여줌
		}
		// ************** DEBUG **************
...
$obj_play = new Play();    // 처음 인스톨
$obj_play->game_start();   // 이제 시작
// $obj_play->set_card();
...
class DebugBJ
{
	...
	public static function arr_to_string( $param_arr )
	{
		...
		foreach( $param_arr as $key => $val )
		{
			if( is_array( $val ) )
			{
				$str .= DebugBJ::arr_to_string( $val );                   // 재귀함수 써서 array 배열이 몇차원 배열일지 모르기때문에 계속 돌려서 문자열만 있으면 그때 값 출력
			}
			else
			{
				$str .= $key." : ".$val."\n";
