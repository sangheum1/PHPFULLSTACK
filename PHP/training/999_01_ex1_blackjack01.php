...
class BlackJack
{
    ...
    public function deck_draw() // 카드 뽑고 저장 (둘다 받는것)
    {
    ...
    public function deck_add_user() // 유저 추가 딜러 추가
    {
        if ( $this->user_sum < 21 && $this->dealer_sum < 21 )
        {
            $this->deck_draw_user[] = $this->arr_deck[$this->deck_count];
            $this->deck_count--;
            if ( $this->dealer_sum < 17 )
            {
                $this->deck_draw_dealer[] = $this->arr_deck[$this->deck_count];
                $this->deck_count--;
            }
    ...
    // public function deck_compare_add()
    // {
    //     $this->dealer_sum > 
    // }
    ...
    {
        if ( ($this->user_sum == 21 && $this->dealer_sum <=21) || ($this->dealer_sum == 21 && $this->user_sum <= 21) )
        {
            if ( $this->user_sum == 21 && $this->dealer_sum == 21 )
            {
                echo "무승부";
            }
            else if ( $this->dealer_sum < 21 )
            {
                echo "유저 승리";
            }
            else
        ...
        else if ( ($this->user_sum > 21 || ($this->dealer_sum > 21 && $this->user_sum <= 21) ))
        {
            if ( $this->user_sum > 21 )
            {
                echo "딜러 승리";
        ...
        // else if ( $this->user_sum > 21)

        else
        {
            if ( $this->user_sum > $this->dealer_sum )
            {
                echo "유저 승리";
            }
            else if ( $this->user_sum < $this->dealer_sum )
            {
                echo "딜러 승리";
            }
            else
...
$obj_bj->deck_value();
$obj_bj->dealer_sum();
$obj_bj->deck_add_user();

$obj_bj->deck_value();
$obj_bj->dealer_sum();


$obj_bj->print_win_lose();
